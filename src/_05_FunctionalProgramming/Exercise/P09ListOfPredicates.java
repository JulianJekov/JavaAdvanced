...
public class P09ListOfPredicates {
    public static void main(String[] args) {
        ...
        List<Integer> numbersToDivideTo = Arrays.stream(scanner.nextLine().split("\\s+")).map(Integer::parseInt).collect(Collectors.toList());

        Predicate<Integer> isDivisible = number -> {
            for (Integer integer : numbersToDivideTo) {
                if(number % integer != 0){
                    return false;
                }
            }
